...
class GRPOTrainer(Trainer):
    ...
    def _get_per_token_logps(self, model, input_ids, attention_mask, logits_to_keep):
        ...
        # Ink: input_ids is actually the id of completion tokens. We get the log(p()) of each completion by their ids
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for stage_id in range(num_stages):
            ...
            prompts = prompt_completion_pairs[stage_id]['prompt_messages']
            prompt_mask = torch.ones(len(prompts), len(prompt_ids), dtype=torch.int64, device=device)
            prompts_text = prompt_completion_pairs[stage_id]['prompt_texts']
            ...
            completion_mask = (sequence_indices <= eos_idx.unsqueeze(1)).int() # 1 is valid
    ...
    def compute_loss(self, model, inputs_list, return_outputs=False, num_items_in_batch=None):
        ...
        if (len(inputs_list) == 0):
            ...
            logits_to_keep = completion_ids.size(1)  # we only need to compute the logits for the completion tokens
            # Ink: This part contributes to the KL loss. Recall that the reference logps is enclosed in torch.inference_mode()
            # NOTE: Pay attention to the size of per_token_logps, which corresponds to the tokens contributing to the gradient
            ...
            # NOTE: This part is not modified
            # Log the metrics
            completion_length = self.accelerator.gather_for_metrics(completion_mask.sum(1)).float().mean().item()
            self._metrics[f"completion_length-stage_{stage_idx}"].append(completion_length)

            mean_kl = ((per_token_kl * completion_mask).sum(dim=1) / completion_mask.sum(dim=1)).mean()
            self._metrics[f"kl-stage_{stage_idx}"].append(self.accelerator.gather_for_metrics(mean_kl).mean().item())
